...
public sealed partial class CalculatorService
{
	public async Task<double> EvaluateAsync(string input)
	{
		...
		var exp = new Expression(exprText)
		{
			Parameters =
			{
				["pi"] = Math.PI,
	...
	private static async Task<double> EvalAsync(LogicalExpression expr)
	{
		//var val = await expr.EvaluateAsync();
		return Convert.ToDouble(0 /*val */, CultureInfo.InvariantCulture);
	}
